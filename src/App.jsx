...
const dfmt = (d) => `${d.getMonth() + 1}월 ${d.getDate()}일`
const dadd = (d, n) => {
  const x = new Date(d)
...
function Input({ label, required, hint, error, full = false, children }) {
  return (
...
function ReviewScreen({ plan, onEdit, onAnalyze }) {
...
function LoadingScreen({ plan, onDone }) {
...
function ResultScreen({ A, onEdit, onReport, onOpen, openKey }) {
  const rows = [
    ['목표 방문객 타당성', A.v.s1],
    ['트렌드 핏', A.v.s2],
    ['지역·시기 관광수요', A.v.s3],
...
function Panel({ item, A, onClose, onEdit, onMove }) {
  if (!item) return null
  const d = cardData(item, A)
  return (
    <>
...
function ReportScreen({ A, onBack, onPrint }) {
